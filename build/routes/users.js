...
router.get('/:profile_id', (req, res) => {
  // Get all the user's informations.
...
router.put('/changePassword/:token', (req, res) => {
  if (!(0, checkBody_1.default)(req.body, ['currentPassword', 'newPassword'])) {
    // Check if current & new password are both given by user in frontend
    res.json({ result: false, error: 'Empty or missing fields.' });
    return;
  }
  User.findOne({ token: req.params.token }).then((userData) => {
    // If all fields are completed in frontend, find the user by his token registered in DB
    if (!userData) {
      ...
      // Check if new password match current password registered in DB
      ...
      // Check if current password match the one registered in DB
      const hash = bcrypt.hashSync(req.body.newPassword, 10);
      User.updateOne({ token: req.params.token }, { password: hash }).then((data) => {
        // If everything is OK, password is modified
        if (data) {
          res.json({ result: true, data: data });
        } else {
      ...
      // If current password doesn't match the one registered in DB, user is informed.
